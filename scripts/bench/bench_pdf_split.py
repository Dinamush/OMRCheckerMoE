...
Usage::
    ...
    python scripts/bench/bench_pdf_split.py

    # Bigger run, with comparison between in-memory and legacy pipelines.
    python scripts/bench/bench_pdf_split.py --pages 1000 --compare

The benchmark uses an existing preset (``MoE-April-2026-Landscape-NNQ25-0``) so
...
def _benchmark(name: str, *, pages: int, source_image: Path, inmemory: bool) -> dict:
    ...
    preset_dir = REPO_ROOT / "MoE-April-2026-Landscape-NNQ25-0"
...
def main() -> None:
    ...
    ap.add_argument(
        "--source",
        type=Path,
        default=REPO_ROOT / "MoE-April-2026-Landscape-NNQ25-0" / "inputs" /
