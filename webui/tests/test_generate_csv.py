...
def test_generate_csv_omits_output_file_by_default(client: TestClient) -> None:
    ...
    assert payload["include_output_file"] is False
    ...
    assert len(rows) == 3
    assert "output_file" not in rows[0]
...
def test_generate_csv_includes_output_file_when_requested(client: TestClient) -> None:
    response = client.post(
        "/api/v1/generate-csv",
        data={
            "count": "2",
    ...
    assert response.status_code == 200, response.text
    payload = response.json()
    assert payload["include_output_file"] is True
    ...
    assert rows[0]["output_file"] == "student_1.png"
