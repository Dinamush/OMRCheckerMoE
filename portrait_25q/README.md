# `portrait_25q/` — Portrait-orientation 25-question OMR sheet (research + design)
...
> spike, not a drop-in replacement for `custom_25_definitive_final/`.
...
The existing 25Q sheet (`custom_25_definitive_final/template.json`) is landscape:
...
This rewrites `reference/blank_portrait_25q.png` from scratch using the
...
`scripts/calibrate_bubbles.py` (adapted from the landscape calibration flow
described in [`docs/audit_report_20260524.md`](../docs/audit_report_20260524.md)).

## How to run OMR against this template (once calibrated)

```bash
python main.py -i portrait_25q/ -o portrait_25q/outputs
```

…with calibrated, pre-filled scans dropped into `portrait_25q/inputs/`.

## Next steps before this can replace the landscape sheet

1. Calibrate `template.json` against the generated reference using
   `scripts/calibrate_bubbles.py` (see `docs/audit_report_20260524.md`).
