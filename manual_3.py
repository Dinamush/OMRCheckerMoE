...
def parse_video_links(html):
    ...
    # You should verify and update these selectors as necessary
    ...
    for request in driver.requests:
        if request.response:
            ...
            if "video" in content_type:
                ...
                logging.info(f"Captured video URL: {request.url}")
    return media_urls
    ...
    while attempt < retries:
        try:
            ...
            if attempt == retries:
                ...
                logging.info(f"Retrying download ({attempt}/{retries})...")
...
def process_video(driver, session, idx, video_page_url):
    try:
        ...
        logging.info(f"Processing video page: {video_page_url}")
        ...
        try:
            ...
            if play_button:
                ...
                time.sleep(5)  # Wait for media requests to be captured
        except Exception as e:
        ...
        if media_urls:
            for media_idx, media_url in enumerate(media_urls):
                ...
                filepath = os.path.join(DOWNLOAD_DIR, f"video_{idx + 1}_{media_idx + 1}.mp4")
    ...
    try:
        ...
        if video_page_links:
            ...
            for idx, video_page_url in enumerate(video_page_links):
                process_video(driver, session, idx, video_page_url)
        else:
...
if __name__ == "__main__":
    ensure_download_dir()  # Ensure the download directory is created before starting
    main()
